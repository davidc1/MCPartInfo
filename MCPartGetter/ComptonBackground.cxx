    ...
    */
	...
	if (PartTraj.size() > 0){
	  ...
	  if ( !(result.at(j).isPrimary()) ){
		...
		_PoCAtoAncestor = _PoCA.ClosestApproachToTrajectory(AncestorTraj,partOrigin,partStart,c1,c2,t1,t2);
	  ...
	  for (size_t y=0; y < muonTracks.size(); y++){
	    ...
	    double tmpPoka = _PoCA.ClosestApproachToTrajectory(muonTracks.at(y),partOrigin,partEnd,c1,c2,t1,t2);
