#ifndef MCSHOWERTEST_CXX
#define MCSHOWERTEST_CXX
...
namespace larlite {
  ...
  bool MCShowerTest::analyze(storage_manager* storage) {
	...
	std::vector<double> shrStartmc = {_Xmc, _Ymc, _Zmc};
	...
	double shrMommc = sqrt(_Pxmc*_Pxmc+_Pymc*_Pymc+_Pzmc*_Pzmc);
	std::vector<double> shrDir = {_Px/shrMom,_Py/shrMom,_Pz/shrMom};
	std::vector<double> shrDirmc = {_Pxmc/shrMommc,_Pymc/shrMommc,_Pzmc/shrMommc};
	...
	if ( (abs(_ancestorPDG) == 11) or (abs(_ancestorPDG) == 13) or
	  ...
	  for (size_t m=0; m < evt_mctracks->size(); m++)
		...
		_cutParamCalculator.getNearestMuonParams(&shrStart, &shrDir, &tracks, &ID, -1, _ancDist, _ancIP, _ancToIP);
		_cutParamCalculator.getNearestMuonParams(&shrStartmc, &shrDirmc, &tracks, &ID, -1, _ancDistmc, _ancIPmc, _ancToIPmc);
  ...
  double MCShowerTest::addTrack(mctrack track){
  ...
  void MCShowerTest::prepareTree(){
    ...
    if(!_ana_tree) {
      ...
      _ana_tree->Branch("_Xmc",&_Xmc,"Xmc/D");//---------------------G4 start X point of mcparticle / first energy dep. point of shower
      _ana_tree->Branch("_Ymc",&_Ymc,"Ymc/D");//---------------------G4 start Y point of mcparticle / first energy dep. point of shower
      _ana_tree->Branch("_Zmc",&_Zmc,"Zmc/D");//---------------------G4 start Z point of mcparticle / first energy dep. point of shower
      ...
      _ana_tree->Branch("_Pxmc",&_Pxmc,"Pxmc/D");//------------------G4 start Px point of mcparticle / first energy dep. point of shower
      _ana_tree->Branch("_Pymc",&_Pymc,"Pymc/D");//------------------G4 start Py point of mcparticle / first energy dep. point of shower
      _ana_tree->Branch("_Pzmc",&_Pzmc,"Pzmc/D");//------------------G4 start Pz point of mcparticle / first energy dep. point of shower
      ...
      _ana_tree->Branch("_ancDistmc",&_ancDistmc,"ancDistmc/D");//----Distance from {_X,_Y,_Z} to ancestor (if ancestor is e+/e-/mu+/mu-/p/pi+/pi-)
      _ana_tree->Branch("_ancIPmc",&_ancIPmc,"ancIPmc/D");//----------Impact Param to ancestor (if ancestor is e+/e-/mu+/mu-/p/pi+/pi-)
      _ana_tree->Branch("_ancToIPmc",&_ancToIPmc,"ancToIPmc/D");//----dist between IP and start (if ancestor is e+/e-/mu+/mu-/p/pi+/pi-)
      ...
      _ana_tree->Branch("_ancTrackFound",&_ancTrackFound,"ancTrackFound/I");
