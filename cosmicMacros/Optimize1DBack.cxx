...
void Optimize1DBack(){
  ...
  sensOptimal /= nBins;
  std::cout << "Optimal Chi^2: " << sensOptimal << std::endl;

  TH1D *hOpt = new TH1D("hOpt",Form("Significance of Signal - Diff. w.r.t. Signal Only Significance [%.2lf]; Cut on Dist. Back-to-Wall [cm]",sensOptimal),
  ...
  for (int dCut = 0; dCut < Nscans; dCut++){
    ...
    siglTree->Draw("E>>hECylBackSigl",Form("inActiveVolume==1 && distBackAlongTraj>%g %s",Cut,fidVol.c_str()));
    ...
    bkgdTree->Draw("E>>hECylBackBkgd",Form("inActiveVolume==1 && distBackAlongTraj>%g %s",Cut,fidVol.c_str()));
    ...
    */
    ...
    std::cout << "new chi^2: " << opt << std::endl;
